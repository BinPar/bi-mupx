...
function copyStartScriptAndDeployVerify(taskList, env, config, appName, deployCheckWaitTime, port, nDeploy) {
  const nDeployStr = (nDeploy && nDeploy > 1 ? nDeploy.toString() : '');
	taskList.copy(nDeploy + '. Initializing start script port ' + port, {
		src: path.resolve(TEMPLATES_DIR, 'start.sh'),
		dest: '/opt/' + appName + '/config/start' + nDeployStr + '.sh',
		vars: {
			appName: appName,
			...
			additionalDockerRunOptions: config.additionalDockerRunOptions,
      ...
      dockerName: appName + nDeployStr,
		}
	});
  if (nDeploy && nDeploy > 1) {
    startAndVerify(taskList, appName, port, deployCheckWaitTime, nDeploy);
  } else {
...
exports.stop = function (config) {
  ...
  for (var i = 0; i < ports.length; i += 1) {
    ...
    if (port) {
      ...
      var nDeploy = i + 1;
      const nDeployStr = (nDeploy && nDeploy > 1 ? nDeploy.toString() : '');
      taskList.executeScript(nDeploy + '. Stopping app port ' + port, {
        script: path.resolve(SCRIPT_DIR, 'stop.sh'),
        vars: {
          appName: config.appName + nDeployStr
        }
      });
    }
  }
...
function startAndVerify(taskList, appName, port, deployCheckWaitTime, nDeploy) {
  const nDeployStr = (nDeploy && nDeploy > 1 ? nDeploy.toString() : '');
	taskList.execute(nDeploy + '. Starting app port ' + port, {
		command: 'bash /opt/' + appName + '/config/start' + nDeployStr + '.sh'
	});

	// verifying deployment
	taskList.executeScript(nDeploy + '. Verifying deployment port ' + port, {
...
function deployAndVerify(taskList, appName, port, deployCheckWaitTime, isNextJS, mupxNodeVersion, nDeploy) {
  ...
  const nDeployStr = (nDeploy && nDeploy > 1 ? nDeploy.toString() : '');
	if(isNextJS) {
		taskList.executeScript(nDeploy + '. Invoking deployment process (Node) port ' + port, {
		...
		taskList.executeScript(nDeploy + '. Invoking deployment process (Meteor) port ' + port, {
